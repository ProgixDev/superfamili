...
export default function EducateurMessagesPage() {
  ...
  // Fetch conversations
  const { data: conversationsData, isLoading: convoLoading } = useQuery<any>({
  ...
  const { data: messagesData, isLoading: messagesLoading } = useQuery<any>({
    queryKey: ["educator-messages", selectedConversationId],
    queryFn: () => apiGet(`/messaging/conversations/${selectedConversationId}/messages`),
    enabled: !!selectedConversationId,
    refetchInterval: 5000,
  })

  const messages: Message[] = messagesData?.data || (Array.isArray(messagesData) ? messagesData : [])
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["educator-conversations"] })
    },
  })

  // Send message using parent_profile_id
  const sendMutation = useMutation({
    mutationFn: (params: { parentProfileId: string; content: string }) =>
      apiPost(`/messaging/conversations/${params.parentProfileId}/messages`, {
        content: params.content,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["educator-messages", selectedConversationId] })
      queryClient.invalidateQueries({ queryKey: ["educator-conversations"] })
      setMessageInput("")
    },
  })
  ...
  const handleSelectConversation = (convo: Conversation) => {
    ...
    if (convo.educator_unread_count > 0) {
  ...
  const handleSend = () => {
    ...
    sendMutation.mutate({
      parentProfileId: activeConversation.parent_profile_id,
  ...
  const getParentName = (convo: Conversation): string => {
    const profile = convo.parent_profiles?.profiles
          ...
          {convoLoading ? (
            ...
            conversations.map((convo) => (
              ...
              >
                ...
                {convo.educator_unread_count > 0 && (
                  <Badge className="shrink-0 bg-[#2E7D52] text-white">
                    {convo.educator_unread_count}
                  </Badge>
                )}
              </button>
            ))
            ...
            <div className="flex items-center gap-3 border-b border-[#E8E4DF] px-4 py-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-[#1A1A1A]">
                  {getParentName(activeConversation)}
                </p>
              </div>
            </div>
