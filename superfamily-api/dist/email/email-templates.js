...
exports.emailChangeEmail = emailChangeEmail;
exports.emailChangeNoticeEmail = emailChangeNoticeEmail;
const COLORS = {
    primary: '#2E7D52',
    primaryDark: '#1B5E38',
    text: '#1C2B20',
...
function baseLayout(chrome, inner, previewText) {
    return `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="utf-8">
...
function signupVerificationEmail(chrome, params) {
    ...
    const inner = [
        ...
        paragraph(greeting),
        paragraph(`Merci de vous \u00eatre inscrit sur ${escapeHtml(chrome.fromName)}. Voici votre code de v\u00e9rification \u00e0 six chiffres\u00a0:`),
        otpBlock(params.code),
        paragraph(`Ce code expire dans <strong>${params.expiresInMinutes} minutes</strong>.`),
        mutedLine("Vous n'\u00eates pas \u00e0 l'origine de cette inscription\u00a0? Ignorez simplement ce message."),
    ].join('');
    return baseLayout(chrome, inner, `Votre code de v\u00e9rification ${chrome.fromName}: ${params.code}`);
}
function passwordResetEmail(chrome, params) {
    const greeting = params.firstName
        ? `Bonjour ${escapeHtml(params.firstName)},`
        : 'Bonjour,';
    const inner = [
        ...
        paragraph(greeting),
        paragraph('Nous avons re\u00e7u une demande de r\u00e9initialisation de votre mot de passe. Utilisez le code ci-dessous pour en d\u00e9finir un nouveau\u00a0:'),
        otpBlock(params.code),
        paragraph(`Ce code expire dans <strong>${params.expiresInMinutes} minutes</strong>.`),
        mutedLine("Vous n'avez pas demand\u00e9 de r\u00e9initialisation\u00a0? Votre mot de passe reste inchang\u00e9\u00a0; vous pouvez ignorer ce message."),
    ].join('');
    return baseLayout(chrome, inner, `Code de r\u00e9initialisation: ${params.code}`);
}
function emailChangeEmail(chrome, params) {
    const greeting = params.firstName
        ? `Bonjour ${escapeHtml(params.firstName)},`
        : 'Bonjour,';
    const inner = [
        ...
        paragraph(greeting),
        paragraph(`Vous avez demand\u00e9 \u00e0 changer l'adresse courriel de votre compte ${escapeHtml(chrome.fromName)} vers <strong>${escapeHtml(params.newEmail)}</strong>. Entrez ce code pour confirmer le changement\u00a0:`),
        otpBlock(params.code),
        paragraph(`Ce code expire dans <strong>${params.expiresInMinutes} minutes</strong>.`),
        mutedLine("Vous n'\u00eates pas \u00e0 l'origine de cette demande\u00a0? Ignorez ce message et, si vous le souhaitez, changez votre mot de passe par pr\u00e9caution."),
    ].join('');
    return baseLayout(chrome, inner, `Confirmez votre nouvelle adresse: ${params.code}`);
}
function emailChangeNoticeEmail(chrome, params) {
    const greeting = params.firstName
        ? `Bonjour ${escapeHtml(params.firstName)},`
        : 'Bonjour,';
    const inner = [
        ...
        paragraph(greeting),
        paragraph(`L'adresse courriel associ\u00e9e \u00e0 votre compte ${escapeHtml(chrome.fromName)} a \u00e9t\u00e9 remplac\u00e9e par <strong>${escapeHtml(params.newEmail)}</strong>.`),
        mutedLine("Si ce n'est pas vous qui avez effectu\u00e9 ce changement, contactez imm\u00e9diatement notre \u00e9quipe de support."),
    ].join('');
    return baseLayout(chrome, inner, `Votre adresse ${chrome.fromName} a \u00e9t\u00e9 modifi\u00e9e`);
}
